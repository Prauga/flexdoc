...
function stats(latencies, count, elapsedMs, parallelism) {
  return {
    ...
    maxMs: round(latencies.reduce((maximum, value) => Math.max(maximum, value), 0)),
  };
}
...
function directRequest(appOrigin) {
  return () => timedFetch(`${appOrigin}/target`);
}
...
async function runSustainedScenario(scenario, port, requestFactory) {
  return withFixture(scenario, 'host', port, async ({ appOrigin, child, startupMs, docsPrimeMs }) => {
    const preActiveIdle = await idleSample(child.pid);
    const execute = requestFactory(appOrigin);
...
process.stdout.write(`Benchmarking ${label} / directActive (${sustainedSeconds}s @ c1 + ${sustainedSeconds}s @ c12)... `);
scenarios.directActive = await runSustainedScenario('directActive', basePort + definitions.length, directRequest);
console.log('done');

process.stdout.write(`Benchmarking ${label} / hostActive (${sustainedSeconds}s @ c1 + ${sustainedSeconds}s @ c12)... `);
scenarios.hostActive = await runSustainedScenario('hostActive', basePort + definitions.length + 1, hostRequest);
console.log('done');

const baseline = scenarios.baseline;
const flexdoc = scenarios.flexdoc;
const hostIdle = scenarios.hostIdle;
const directActive = scenarios.directActive;
const hostActive = scenarios.hostActive;

const directWarmRssKiB = directActive.warmedIdle.rssKiB - directActive.preActiveIdle.rssKiB;
const directWarmPssKiB = directActive.warmedIdle.pssKiB - directActive.preActiveIdle.pssKiB;
const directC1PeakRssKiB = directActive.sustainedConcurrency1.peakRssKiB - directActive.preActiveIdle.rssKiB;
const directC1PeakPssKiB = directActive.sustainedConcurrency1.peakPssKiB - directActive.preActiveIdle.pssKiB;
const directC12PeakRssKiB = directActive.sustainedConcurrency12.peakRssKiB - directActive.preActiveIdle.rssKiB;
const directC12PeakPssKiB = directActive.sustainedConcurrency12.peakPssKiB - directActive.preActiveIdle.pssKiB;
const directCooldownRssKiB = directActive.cooldownFinal.rssKiB - directActive.preActiveIdle.rssKiB;
const directCooldownPssKiB = directActive.cooldownFinal.pssKiB - directActive.preActiveIdle.pssKiB;

const hostWarmRssKiB = hostActive.warmedIdle.rssKiB - hostActive.preActiveIdle.rssKiB;
const hostWarmPssKiB = hostActive.warmedIdle.pssKiB - hostActive.preActiveIdle.pssKiB;
const hostC1PeakRssKiB = hostActive.sustainedConcurrency1.peakRssKiB - hostActive.preActiveIdle.rssKiB;
const hostC1PeakPssKiB = hostActive.sustainedConcurrency1.peakPssKiB - hostActive.preActiveIdle.pssKiB;
const hostC12PeakRssKiB = hostActive.sustainedConcurrency12.peakRssKiB - hostActive.preActiveIdle.rssKiB;
const hostC12PeakPssKiB = hostActive.sustainedConcurrency12.peakPssKiB - hostActive.preActiveIdle.pssKiB;
const hostCooldownRssKiB = hostActive.cooldownFinal.rssKiB - hostActive.preActiveIdle.rssKiB;
const hostCooldownPssKiB = hostActive.cooldownFinal.pssKiB - hostActive.preActiveIdle.pssKiB;
...
const result = {
  ...
  deltas: {
    ...
    directActiveWarmRssKiB: directWarmRssKiB,
    directActiveWarmPssKiB: directWarmPssKiB,
    directActiveC1PeakOverIdleRssKiB: directC1PeakRssKiB,
    directActiveC1PeakOverIdlePssKiB: directC1PeakPssKiB,
    directActiveC12PeakOverIdleRssKiB: directC12PeakRssKiB,
    directActiveC12PeakOverIdlePssKiB: directC12PeakPssKiB,
    directActiveCooldownRssKiB: directCooldownRssKiB,
    directActiveCooldownPssKiB: directCooldownPssKiB,

    hostActiveWarmRssKiB: hostWarmRssKiB,
    hostActiveWarmPssKiB: hostWarmPssKiB,
    hostActiveC1PeakOverIdleRssKiB: hostC1PeakRssKiB,
    hostActiveC1PeakOverIdlePssKiB: hostC1PeakPssKiB,
    hostActiveC12PeakOverIdleRssKiB: hostC12PeakRssKiB,
    hostActiveC12PeakOverIdlePssKiB: hostC12PeakPssKiB,
    hostActiveCooldownRssKiB: hostCooldownRssKiB,
    hostActiveCooldownPssKiB: hostCooldownPssKiB,

    hostAttributableWarmRssKiB: hostWarmRssKiB - directWarmRssKiB,
    hostAttributableWarmPssKiB: hostWarmPssKiB - directWarmPssKiB,
    hostAttributableC1PeakRssKiB: hostC1PeakRssKiB - directC1PeakRssKiB,
    hostAttributableC1PeakPssKiB: hostC1PeakPssKiB - directC1PeakPssKiB,
    hostAttributableC12PeakRssKiB: hostC12PeakRssKiB - directC12PeakRssKiB,
    hostAttributableC12PeakPssKiB: hostC12PeakPssKiB - directC12PeakPssKiB,
    hostAttributableCooldownRssKiB: hostCooldownRssKiB - directCooldownRssKiB,
    hostAttributableCooldownPssKiB: hostCooldownPssKiB - directCooldownPssKiB,
    ...
    directSustainedC1P95Ms: directActive.sustainedConcurrency1.p95Ms,
    directSustainedC1ThroughputRps: directActive.sustainedConcurrency1.throughputRps,
    directSustainedC1CpuMsPerRequest: directActive.sustainedConcurrency1.cpuMsPerRequest,
    directSustainedC12P95Ms: directActive.sustainedConcurrency12.p95Ms,
    directSustainedC12ThroughputRps: directActive.sustainedConcurrency12.throughputRps,
    directSustainedC12CpuMsPerRequest: directActive.sustainedConcurrency12.cpuMsPerRequest,
