...
func optionsFromHTML(t *testing.T, body string) map[string]any {
	...
	if start < 0 {
		t.Fatal("renderer options missing")
	}
	rest := body[start+len(prefix):]
	end := strings.Index(rest, ";</script>")
	if end < 0 {
		t.Fatal("renderer options terminator missing")
	}
	var options map[string]any
	if err := json.Unmarshal([]byte(rest[:end]), &options); err != nil {
		t.Fatalf("decode renderer options: %v", err)
	}
	return options
}
...
func TestHandlerServesBundledAssets(t *testing.T) {
	...
	if rec.Code != 200 || rec.Body.Len() == 0 {
		t.Fatal("bundled renderer asset unavailable")
	}
}
...
func TestHandlerWithAssetsIsSafe(t *testing.T) {
	...
	if !strings.Contains(body, "/reference/__flexdoc/renderer.js?v=") {
		t.Fatal("versioned renderer route missing")
	}
	if !strings.Contains(body, "/reference/__flexdoc/renderer.css?v=") {
		t.Fatal("versioned renderer stylesheet missing")
	}
	if strings.Contains(body, "</script><script>alert(1)</script>") {
		t.Fatal("unsafe title rendered")
	}
	if strings.Contains(body, `"/openapi.json?x=</script>"`) {
		t.Fatal("unsafe spec URL rendered")
	}
}
...
func TestRendererOptionsOmitOptionalFields(t *testing.T) {
	...
	if _, ok := options["expand"]; ok {
		t.Fatal("expand should be omitted")
	}
	tryIt := options["tryIt"].(map[string]any)
	if tryIt["enabled"] != true {
		t.Fatalf("tryIt.enabled = %#v", tryIt["enabled"])
	}
	if len(tryIt) != 1 {
		t.Fatalf("unexpected Try It options: %#v", tryIt)
	}
}
...
func TestRendererOptionsSerializeExpandAndTryItSettings(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if string(actualExpand) != string(expectedExpand) {
				t.Fatalf("expand = %s, want %s", actualExpand, expectedExpand)
			}
			tryIt := options["tryIt"].(map[string]any)
			if tryIt["enabled"] != true || tryIt["defaultServer"] != "https://gateway.example.test" || tryIt["credentials"] != "include" {
				t.Fatalf("Try It options = %#v", tryIt)
			}
			if value, ok := tryIt["apiClientPersistenceKey"].(bool); !ok || value {
				t.Fatalf("persistence key = %#v", tryIt["apiClientPersistenceKey"])
			}
			hostExecution := tryIt["hostExecution"].(map[string]any)
			if hostExecution["available"] != false || hostExecution["endpoint"] != "/reference/__flexdoc/execute" {
				t.Fatalf("host execution = %#v", hostExecution)
			}
			if capabilities, ok := hostExecution["capabilities"].([]any); !ok || len(capabilities) != 0 {
				t.Fatalf("host capabilities = %#v", hostExecution["capabilities"])
			}
			execute := httptest.NewRecorder()
			h.ServeHTTP(execute, httptest.NewRequest("POST", "/reference/__flexdoc/execute", nil))
			if execute.Code != 404 {
				t.Fatalf("unsupported execute route status = %d", execute.Code)
			}
		})
	}
}
...
func TestHandlerFromOpenAPIServesGeneratedSpecAndDocs(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("HandlerFromOpenAPI: %v", err)
	}
	...
	if !strings.Contains(page.Body.String(), "/reference/__flexdoc/openapi.json") {
		t.Fatal("docs page does not reference generated spec route")
	}

	generated := httptest.NewRecorder()
	h.ServeHTTP(generated, httptest.NewRequest("GET", "/reference/__flexdoc/openapi.json", nil))
	if generated.Code != 200 {
		t.Fatalf("spec status = %d", generated.Code)
	}
	if got := generated.Header().Get("Content-Type"); !strings.HasPrefix(got, "application/json") {
		t.Fatalf("content-type = %q", got)
	}
	var decoded map[string]any
	if err := json.Unmarshal(generated.Body.Bytes(), &decoded); err != nil {
		t.Fatalf("decode generated spec: %v", err)
	}
	if decoded["openapi"] != "3.1.0" {
		t.Fatalf("openapi = %#v", decoded["openapi"])
	}
}
