...
func HandlerWithAssets(cfg Config, assets fs.FS) http.Handler {
	return handlerWithSpec(cfg, assets, nil)
}
...
func handlerWithSpec(cfg Config, assets fs.FS, spec []byte) http.Handler {
	...
	if len(spec) > 0 {
		cfg.SpecURL = strings.TrimRight(cfg.Path, "/") + "/__flexdoc/openapi.json"
	}
	if cfg.Title == "" {
	...
	if len(spec) > 0 {
		cfg.SpecURL = cfg.Path + "/__flexdoc/openapi.json"
	}
	return &handler{
		cfg:             cfg,
		assets:          assets,
		spec:            spec,
		rendererVersion: assetVersion(assets),
		hostExecution:   cfg.HostExecution,
	}
}
...
func (h *handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case h.cfg.Path + "/__flexdoc/openapi.json":
		...
		_, _ = w.Write(h.spec)
	case h.cfg.Path + "/__flexdoc/execute":
		if !h.cfg.TryItHostExecution || h.hostExecution == nil {
			http.NotFound(w, r)
			return
		}
		h.hostExecution.ServeHTTP(w, r)
	default:
		http.NotFound(w, r)
	}
}
...
func (h *handler) asset(w http.ResponseWriter, name, contentType string) {
	...
	if err != nil {
		http.Error(w, "FlexDoc renderer asset is unavailable", http.StatusInternalServerError)
		return
	}
...
func (h *handler) html() string {
	...
	if h.cfg.TryItDefaultServer != "" {
		tryIt["defaultServer"] = h.cfg.TryItDefaultServer
	}
	if h.cfg.TryItCredentials != "" {
		tryIt["credentials"] = h.cfg.TryItCredentials
	}
	if h.cfg.TryItAPIClientPersistenceKey != nil {
		tryIt["apiClientPersistenceKey"] = h.cfg.TryItAPIClientPersistenceKey
	}
	if h.cfg.TryItHostExecution {
		available := h.hostExecution != nil
		capabilities := []string{}
		if available {
			capabilities = h.hostExecution.Capabilities()
		}
