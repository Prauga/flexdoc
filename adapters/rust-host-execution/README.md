...
The default `host-execution` feature owns the outbound HTTP stack. Building with `default-features = false` produces a transport-free crate and does not pull reqwest or its TLS stack; the framework adapters use that mode when native host execution is compiled out.
