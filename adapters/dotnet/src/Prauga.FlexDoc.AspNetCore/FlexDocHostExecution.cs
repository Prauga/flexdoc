...
public sealed class FlexDocHostExecution
{
        ...
        for (var redirect = 0; redirect <= MaxRedirects; redirect++)
        {
            ...
            using (response)
            {
                ...
                try
                {
                    ...
                    responseBody = await ReadLimitedAsync(stream, MaxResponseBytes, timeoutCts.Token, responseLimit: true, knownLength: response.Content.Headers.ContentLength);
                }
                catch (OperationCanceledException) when (!outerCancellationToken.IsCancellationRequested && timeoutCts.IsCancellationRequested)
                {
                    throw Upstream($"Host execution request timed out after {(long)timeout.TotalMilliseconds} ms.", FlexDocHostExecutionReasons.UpstreamTimeout);
    ...
    {
        var initialCapacity = knownLength.HasValue && knownLength.Value >= 0 && knownLength.Value <= maxBytes
            ? (int)knownLength.Value
            : Math.Min(maxBytes, 4 * 1024);
