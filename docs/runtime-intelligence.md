...
`serverOrigin`, `server`, and `environment` are optional. Missing values are omitted rather than guessed. `validation` is also optional at the renderer-contract level so route snapshots without it remain compatible. Node Express/Fastify/Hono/Nest, Spring MVC, ASP.NET Core, and FastAPI produce it.
...
- `runtime.operation-undocumented` — the running backend positively exposes an operation that OpenAPI does not document. Severity: `error` when discovery is complete, `warning` when discovery is partial, matching the other findings that must not treat an incomplete inventory as authoritative. The finding names the method and path and opens that registered route. It is not an OpenAPI operation. The page shows the contract claim — OpenAPI does not declare the operation — beside the registered method and path. It does not invent a handler or a schema. **Open in API Client** sends that method and path on `serverOrigin` when the snapshot has one, and as a same-origin path otherwise. It does not use the OpenAPI server URL. The request asks to prefer the API host only when host execution is available and the server has not set `preferHostExecution` to false. `runtimeIntelligence.acknowledgedUndocumented` keeps the route registered and records the finding as acknowledged info, so that finding does not fail validation. An unacknowledged route still fails when discovery is complete, and `flexdoc validate` exits `1`.
