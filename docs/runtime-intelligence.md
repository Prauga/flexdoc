...
- `runtime.operation-undocumented` — the running backend positively exposes an operation that OpenAPI does not document. Severity: `error` when discovery is complete, `warning` when discovery is partial, matching the other findings that must not treat an incomplete inventory as authoritative. The finding names the method and path and opens that registered route. It is not an OpenAPI operation. **Open in API Client** sends that method and path joined to an origin: the snapshot `serverOrigin` when present, otherwise the origin of the configured or documented server URL. A server base path is not prepended, because discovered runtime paths already include their mount prefix. The request asks to prefer the API host only when host execution is available and the server has not set `preferHostExecution` to false; that opt-out still selects the browser, as it does for a documented operation. When discovery is complete the derived status is `fail`, and `flexdoc validate` exits `1` under the default policy. Set `runtimeIntelligence.acknowledgedUndocumented` to method/path pairs that are intentionally absent from OpenAPI; those operations are omitted from this finding.
