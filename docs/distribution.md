...
| `Prauga.FlexDoc.AspNetCore` | `0.5.4` | `dotnet/v0.5.4` | ASP.NET Core 8+; renderer contract v1; HX-03 protection acknowledgement |
| `com.prauga.flexdoc:flexdoc-jvm` | `0.8.4` | `java/v0.8.4` | Java 17+ framework-neutral renderer host; HX-03 protection acknowledgement |
| `com.prauga.flexdoc:flexdoc-jaxrs` | `0.8.4` | `java/v0.8.4` | Jakarta REST/JAX-RS transport over `flexdoc-jvm` |
| `com.prauga.flexdoc:flexdoc-spring-boot-starter` | `0.8.4` | `java/v0.8.4` | Spring Boot 3 transport over `flexdoc-jvm`; HX-03 property binding |
...
| `github.com/prauga/flexdoc/adapters/go` | `0.5.5` | `adapters/go/v0.5.5` | net/http adapter + embedded renderer |
...
Versions are intentionally independent across ecosystems. FlexDoc 3.3 advances client/backend to `3.3.0`; the HX-03 3.3.x security-parity follow-up advances only the Java family to `0.8.4` and ASP.NET Core to `0.5.4` because those package implementations changed. Other native adapters retain the versions already published for the 3.3 release. `@prauga/flexdoc-core` remains `0.5.0` and `@prauga/flexdoc-cli` remains `0.7.0`. The renderer contract, not matching package numbers, remains the cross-ecosystem compatibility boundary.
...
The FlexDoc Java family is prepared together at `0.8.4` for the HX-03 3.3.x security-parity patch:

```text
com.prauga.flexdoc:flexdoc-jvm:0.8.4
com.prauga.flexdoc:flexdoc-jaxrs:0.8.4
com.prauga.flexdoc:flexdoc-spring-boot-starter:0.8.4
```
