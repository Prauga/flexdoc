...
The workspace UI exports the currently active environment by default. Explicit auth credentials follow the workspace credential-storage scope: an export from a session-only or never-store workspace strips credential material from request, folder, and collection auth, while **Remember on this browser** embeds it as entered. Collection variables and environment values are always embedded verbatim and are not covered by that scope, so artifact JSON remains potentially sensitive configuration and must not be committed blindly — a stripped artifact means the credential is absent, not that the file is safe to publish. The workspace shows this warning next to the Runner export controls. 3.2 does not pretend there is already a host-side secret-store protocol; that is separate backend-execution work.
