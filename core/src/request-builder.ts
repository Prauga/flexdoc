...
export interface RequestValues {
  /** Query/path parameter values keyed by OpenAPI parameter name. */
  parameters?: Record<string, RequestValue>;
  /** Explicit request headers keyed by header name. */
  headers?: Record<string, string>;
  /** Cookie parameter values keyed by OpenAPI cookie name. */
  cookies?: Record<string, RequestValue>;
  /** Request body text entered by the user. */
  body?: string;
  /** Media type selected for the request body. */
  contentType?: string;
  /** Security-scheme credential values keyed by OpenAPI scheme name. */
  auth?: Record<string, string>;
  /** Explicit server URL overriding the operation/document server list. */
  serverUrl?: string;
  /** Values substituted into variables of the selected OpenAPI server. */
  serverVariables?: Record<string, string>;
}

/** Canonical fetch-ready request produced from an OpenAPI operation and values. */
export interface BuiltRequest {
  /** Fully resolved request URL including server, path parameters, and query string. */
  url: string;
  /** Fetch-compatible request initialization object. */
  init: RequestInit;
  /** Uppercase HTTP method. */
  method: string;
  /** Header record after parameter and security processing. */
  headers: Record<string, string>;
  /** Serializable body preview when the request has a body. */
  body?: string;
  /** Body representation used by renderer/editor tooling. */
...
function serializePath(parameter: Parameter, value: RequestValue): string {
  ...
  if (Array.isArray(structured)) {
    ...
    if (style === 'matrix') return explode ? items.map((item) => `;${parameter.name}=${item}`).join('') : `;${parameter.name}=${items.join(',')}`;
    return items.join(',');
  }
  if (structured && typeof structured === 'object') {
    const entries = Object.entries(structured).map(([key, item]) => [encode(key), encode(item)] as const);
    if (style === 'label') return explode ? `.${entries.map(([key, item]) => `${key}=${item}`).join('.')}` : `.${entries.flatMap(([key, item]) => [key, item]).join(',')}`;
    if (style === 'matrix') return explode ? entries.map(([key, item]) => `;${key}=${item}`).join('') : `;${parameter.name}=${entries.flatMap(([key, item]) => [key, item]).join(',')}`;
    return explode ? entries.map(([key, item]) => `${key}=${item}`).join(',') : entries.flatMap(([key, item]) => [key, item]).join(',');
  }
...
function serializeQuery(parameter: Parameter, value: RequestValue): Array<[string, string]> {
  ...
  const structured = asStructured(value);
  if (style === 'deepObject' && structured && typeof structured === 'object' && !Array.isArray(structured)) return Object.entries(structured).map(([key, item]) => [`${parameter.name}[${key}]`, primitive(item)]);
  if (Array.isArray(structured)) {
  ...
  if (structured && typeof structured === 'object') {
    ...
    return explode ? objectEntries.map(([key, item]) => [key, primitive(item)]) : [[parameter.name, objectEntries.flatMap(([key, item]) => [key, primitive(item)]).join(',')]];
  }
...
export function buildRequest(spec: OpenAPISpec, path: string, method: string, values: RequestValues = {}): BuiltRequest {
  ...
  for (const parameter of normalized.parameters) {
    const value = parameter.in === 'header' ? values.headers?.[parameter.name] : parameter.in === 'cookie' ? values.cookies?.[parameter.name] : values.parameters?.[parameter.name];
    if (value === undefined || value === '') continue;
    if (parameter.in === 'path') resolvedPath = resolvedPath.replace(`{${parameter.name}}`, serializePath(parameter, value));
    else if (parameter.in === 'query') for (const [key, item] of serializeQuery(parameter, value)) query.push(`${encodeURIComponent(key)}=${encodePart(item, parameter.allowReserved)}`);
  ...
  if (values.body) {
    ...
    if (contentType === 'application/x-www-form-urlencoded') {
      ...
      body = object ? Object.entries(object).flatMap(([key, item]) => Array.isArray(item) ? item.map((entry) => `${encodeURIComponent(key)}=${encodeURIComponent(primitive(entry))}`) : [`${encodeURIComponent(key)}=${encodeURIComponent(primitive(item))}`]).join('&') : values.body;
      requestBody = body;
      bodyKind = 'form';
