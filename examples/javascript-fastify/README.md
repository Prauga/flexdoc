...
The FlexDoc dependency remains pinned to `3.1.0`, the last published release, during 3.0 release preparation. Repository CI validates the example against the backend package built from the same commit; the post-publish lock-refresh change will advance the registry pin.
