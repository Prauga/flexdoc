...
Minimal ASP.NET Core application using `Prauga.FlexDoc.AspNetCore` `0.4.1` and the FlexDoc 3.0 Runtime Intelligence surface.
...
During repository CI the example references the adapter project directly and the adapter embeds the canonical renderer built from `packages/client`. The NuGet version remains `0.4.1` until the coordinated 3.0 release versioning pass.
