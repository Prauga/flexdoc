...
This example uses Hono `4.13.x` with the optional `setupHonoFlexDoc` integration. Its registry dependency remains `@prauga/flexdoc-backend` `3.1.0` during 3.0 release preparation, while repository CI validates the local 3.0 package. Runtime Intelligence reads Hono's actual registered routes instead of inferring them from the OpenAPI document.
