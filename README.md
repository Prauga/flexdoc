...
FlexDoc runs inside the backend and reads the routes the framework actually registered. **On Node, Spring, ASP.NET Core, and FastAPI,** it compares that runtime surface with OpenAPI, fails `flexdoc validate` on unacknowledged disagreement, and lets you open an undocumented runtime route directly in the built-in API Client.
