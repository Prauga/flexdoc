import type { FlexDocHostExecutionCompleteEvent, FlexDocHostExecutionOutcome } from './host-execution-observability';
...
export type FlexDocHostExecutionMetricUpdate =
  ...
  | {
      ...
      kind: 'counter';
      value: 1;
      labels: { source: FlexDocHostExecutionRejectionSource; statusCode: 400 | 403 | 429 };
    }
  | {
...
export function createHostExecutionCompleteMetricUpdates(
  ...
  if (event.outcome === 'rejected' && (event.statusCode === 400 || event.statusCode === 403)) {
    updates.push({
      ...
      kind: 'counter',
      value: 1,
      labels: { source: 'route', statusCode: event.statusCode },
    });
  }
  return updates;
}
