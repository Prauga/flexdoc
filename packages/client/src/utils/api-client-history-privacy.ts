...
export const isSensitiveApiClientHistoryHeader = isSensitiveName;

function redactKeyValues(entries: HttpKeyValue[] | undefined): HttpKeyValue[] | undefined {
...
function redactResponseHeaders(headers: Array<[string, string]> | undefined): Array<[string, string]> | undefined {
  return headers?.map((entry) => isSensitiveName(entry[0])
...
function persistedRequest(request: HttpRequestDraft, omitBody: boolean): HttpRequestDraft {
  const copy = {
    ...
    query: redactKeyValues(request.query),
    headers: redactKeyValues(request.headers),
