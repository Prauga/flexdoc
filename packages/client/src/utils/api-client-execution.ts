...
export interface ApiClientExecutionResponse {
  ...
  /** Browser-to-host round-trip duration for API-host execution. */ hostRoundTripTime?: number;
...
export function apiClientSlowHostHint(
  response: Pick<ApiClientExecutionResponse, 'transport' | 'hostRoundTripTime'> | null | undefined,
  request: HttpRequestDraft,
  hostExecution: FlexDocHostExecutionPublicOptions | undefined,
): string | null {
  const ms = response?.hostRoundTripTime;
  return response?.transport === 'api-host' && ms != null && ms >= API_CLIENT_SLOW_HOST_THRESHOLD_MS
    && resolveApiClientTransport({ request, hostExecution, preferHostExecution: false })[0] === 'browser'
    ? `API-host round trip ${Math.round(ms)} ms. Browser may be faster.`
    : null;
}
