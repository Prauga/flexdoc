...
export interface ApiClientExecutionResponse {
  ...
  /** Browser-to-API-host round-trip duration for host execution. */ hostRoundTripTime?: number;
...
export function apiClientSlowHostHint(
  ...
  const elapsed = response?.hostRoundTripTime ?? response?.responseTime;
  if (response?.transport !== 'api-host' || elapsed == null || elapsed < SLOW_HOST_MS) return null;
  return resolveApiClientTransport({ request, hostExecution, preferHostExecution: false })[0] === 'browser'
    ? `API-host round trip took ${Math.round(elapsed)} ms. Browser may be faster.`
    : null;
}
