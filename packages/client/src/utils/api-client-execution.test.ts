...
console.log('checked');
  ...
  it('guides only materially slow host round trips with viable browser transport', () => {
    const request = { method: 'GET', url: 'https://api.example.test/pets' };
    ...
    expect(apiClientSlowHostHint({ transport: 'api-host', hostRoundTripTime: 731 }, request, host)).toContain('731 ms');
    expect(apiClientSlowHostHint({ transport: 'api-host', hostRoundTripTime: 499 }, request, host)).toBeNull();
  });

  it('suppresses slow-host guidance when browser execution is invalid', () => {
    expect(apiClientSlowHostHint(
      { transport: 'api-host', hostRoundTripTime: 900 },
      { method: 'GET', url: 'https://api.example.test/private', auth: { type: 'digest' as const, username: 'u', password: 'p' } },
