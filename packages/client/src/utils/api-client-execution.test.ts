...
console.log('checked');
  ...
  it('suggests browser-direct comparison only for materially slow API-host round trips', () => {
    const request = { method: 'GET', url: 'https://api.example.test/pets' };
    const hostExecution = { available: true, endpoint: '/docs/__flexdoc/execute', capabilities: [] as [] };

    expect(API_CLIENT_SLOW_HOST_THRESHOLD_MS).toBe(500);
    expect(apiClientSlowHostHint(
      { transport: 'api-host', responseTime: 44, hostRoundTripTime: 731 },
      request,
      hostExecution,
    )).toContain('731 ms');
    expect(apiClientSlowHostHint(
      { transport: 'api-host', responseTime: 44, hostRoundTripTime: 499 },
      request,
      hostExecution,
    )).toBeNull();
  });

  it('suppresses slow-host guidance when the request requires host execution', () => {
    const request = {
      method: 'GET',
      url: 'https://api.example.test/private',
      auth: { type: 'digest' as const, username: 'u', password: 'p' },
    };
    expect(apiClientSlowHostHint(
  ...
  it('suppresses slow-host guidance for browser responses', () => {
    expect(apiClientSlowHostHint(
      { transport: 'browser', responseTime: 900 },
      { method: 'GET', url: 'https://api.example.test/pets' },
      { available: true, endpoint: '/docs/__flexdoc/execute', capabilities: [] },
    )).toBeNull();
  });
