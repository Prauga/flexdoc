import { apiClientCorsFailureHint, apiClientSlowHostHint, executeApiClientRequest } from './api-client-execution';
...
console.log('checked');
  ...
  it('shows slow-host guidance only when browser-direct is viable', () => {
    ...
    expect(apiClientSlowHostHint(
      ...
      { ...request, auth: { type: 'digest' as const, username: 'u', password: 'p' } },
      { ...host, capabilities: ['digest'] },
    )).toBeNull();
  });
