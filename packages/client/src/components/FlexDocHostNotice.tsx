...
export function FlexDocHostNotice({ message, capabilities = [], showCapabilities = true, warning = false, theme = 'light', label = 'Host execution status' }: {
  message: string;
  capabilities?: readonly HttpHostExecutionCapability[];
  /** Hide the capability disclosure for notices unrelated to host execution, such as storage warnings. */
  ...
  return <div role={warning ? 'alert' : 'status'} aria-label={label} className={`rounded-md border p-3 text-sm ${palette}`}>
    <div className='flex items-start gap-2'><FlexDocMark /><div>{message}
      {showCapabilities && <details className='mt-1'><summary>API host capabilities</summary>
