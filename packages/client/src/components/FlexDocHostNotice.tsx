  ...
  return <div role={warning ? 'alert' : 'status'} aria-label={label} className={`rounded-md border p-3 text-sm ${palette}`}>
    <div className='flex items-start gap-2'><FlexDocMark /><div><div>{message}</div>
      <details className='mt-1'>
        <summary>API host capabilities</summary>
        <div className='mt-1 space-y-1 text-xs'>
          <div><strong>Universal</strong> — Standard HTTP execution: method, URL, query, headers, and request body.</div>
          <div><strong>Host-specific</strong> — {capabilities.length > 0 ? capabilities.map(apiClientHostCapabilityLabel).join(' · ') : 'No additional host-specific capabilities advertised.'}</div>
        </div>
      </details>
    </div></div>
  </div>;
