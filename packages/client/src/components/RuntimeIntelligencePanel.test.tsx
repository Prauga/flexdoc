...
describe('RuntimeIntelligencePanel', () => {
  it('shows runtime-only and documented-only routes', () => {
    render(<RuntimeIntelligencePanel open theme='light' loading={false} onClose={() => undefined} snapshot={{
      framework: 'express', discoveryComplete: true, routes: [{ method: 'GET', path: '/pets' }],
      runtimeOnly: [{ method: 'POST', path: '/internal/reindex' }], documentedOnly: [{ method: 'GET', path: '/missing' }],
