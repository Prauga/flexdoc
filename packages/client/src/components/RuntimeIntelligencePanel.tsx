...
export const RuntimeIntelligencePanel: React.FC<Props> = ({ open, theme, loading, error, snapshot, onClose }) => {
  ...
  return <div className='fixed inset-0 z-50'>
    <button tabIndex={-1} className='absolute inset-0 bg-black/40' aria-label='Close runtime intelligence' onClick={onClose} />
    <section
      ref={dialogRef}
      role='dialog'
      aria-modal='true'
    ...
    >
      <div className='mb-5 flex items-start justify-between gap-4'>
        <div><div id='runtime-intelligence-heading' className='flex items-center gap-2 text-lg font-semibold'><Activity className='h-5 w-5' />Runtime intelligence</div><p className={`mt-1 text-sm ${muted}`}>Live route presence from the backend hosting this documentation.</p></div>
        <button ref={closeButtonRef} className='inline-flex h-10 w-10 items-center justify-center rounded-md border' aria-label='Close runtime intelligence panel' onClick={onClose}><X className='h-4 w-4' /></button>
      </div>
      ...
      {snapshot && <div className='space-y-5'>
        <div className='grid grid-cols-2 gap-3 text-sm'>
          ...
          {(snapshot.serverOrigin || snapshot.server?.localPort) && <div className='col-span-2 rounded-lg border p-3'><div className={muted}>Runtime server</div>{snapshot.serverOrigin && <code className='block break-all text-xs'>{snapshot.serverOrigin}</code>}{snapshot.server?.localPort && <div className={`mt-1 text-xs ${muted}`}>Backend listener port {snapshot.server.localPort}</div>}</div>}
          {snapshot.environment?.name && <div className='col-span-2 rounded-lg border p-3'><div className={muted}>Environment</div><div className='font-semibold'>{snapshot.environment.name}</div></div>}
        </div>
        {!snapshot.discoveryComplete && <div className={`flex gap-2 rounded-lg border p-3 text-sm ${warningClasses}`}><AlertTriangle className='mt-0.5 h-4 w-4 shrink-0' />Route discovery is partial; documented routes not observed at runtime may be false positives.</div>}
        ...
        {snapshot.summary.runtimeOnly === 0 && snapshot.summary.documentedOnly === 0 && snapshot.discoveryComplete && <div className={`flex items-center gap-2 text-sm ${successClasses}`}><CheckCircle2 className='h-4 w-4' />Runtime routes and OpenAPI paths are aligned.</div>}
      </div>}
    </section>
  </div>;
};
...
function RouteList({ title, routes, empty, theme, warning = false }: { title: string; routes: Array<{method:string;path:string}>; empty: string; theme: 'light'|'dark'; warning?: boolean }) {
  ...
  return <section><h3 className='mb-2 font-semibold'>{title} <span className={muted}>({routes.length})</span></h3>{routes.length ? <div className='space-y-2'>{routes.map((route) => <div key={`${route.method}:${route.path}`} className={`flex gap-2 rounded border p-2 text-sm ${warningBorder}`}><span className='w-14 shrink-0 font-semibold'>{route.method}</span><code className='break-all'>{route.path}</code></div>)}</div> : <p className={`text-sm ${muted}`}>{empty}</p>}</section>;
}
