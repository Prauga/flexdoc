...
export const ApiClientAuthEditor: React.FC<Props> = ({ auth, label, allowInherit = false, advanced = true, hostExecution, onChange, theme }) => {
  const inputClass = theme === 'dark' ? 'bg-gray-900 border-gray-700 text-gray-100' : 'bg-white border-gray-300 text-gray-900';
  const fieldClass = `w-full rounded-md border ${label ? 'px-2 py-1.5 text-xs' : 'px-3 py-2 text-sm'} ${inputClass}`;
  const named = (collection: string, request = collection) => label ? `${label} ${collection}` : request;
  ...
  return <div className='space-y-2'>
    <select aria-label={named('authorization type', 'Authorization type')} className={fieldClass} value={auth.type === 'inherit' && !allowInherit ? 'none' : auth.type} onChange={(event) => onChange(authForType(event.target.value as HttpAuth['type']))}>
      ...
      {(advanced || auth.type === 'oauth2') && <option value='oauth2'>OAuth 2.0</option>}
      {advanced && <><option value='digest' disabled={!supports('digest')}>Digest (API host)</option><option value='hawk' disabled={!supports('hawk')}>Hawk (API host)</option><option value='ntlm' disabled={!supports('ntlm')}>NTLM / Negotiate (API host)</option><option value='oauth1' disabled={!supports('oauth1')}>OAuth 1.0 (API host)</option><option value='awsv4' disabled={!supports('awsv4')}>AWS Signature v4 (API host)</option></>}
    </select>
    {auth.type === 'bearer' && <input aria-label={named('bearer token', 'Bearer token')} type='password' autoComplete='off' className={fieldClass} value={auth.token} onChange={(event) => onChange({ ...auth, token: event.target.value })} />}
    {auth.type === 'oauth2' && <OAuthEditor auth={auth} fieldClass={fieldClass} label={label} onChange={onChange} />}
    {(auth.type === 'basic' || auth.type === 'digest') && <div className='grid grid-cols-2 gap-2'><input aria-label={named(auth.type + ' username', auth.type === 'basic' ? 'Basic auth username' : 'Digest username')} className={fieldClass} placeholder='Username' value={auth.username} onChange={(event) => onChange({ ...auth, username: event.target.value })} /><input aria-label={named(auth.type + ' password', auth.type === 'basic' ? 'Basic auth password' : 'Digest password')} type='password' autoComplete='off' className={fieldClass} placeholder='Password' value={auth.password} onChange={(event) => onChange({ ...auth, password: event.target.value })} /></div>}
    {auth.type === 'apiKey' && <div className='space-y-2'><input aria-label={named('API key name')} className={fieldClass} placeholder='Key name' value={auth.key} onChange={(event) => onChange({ ...auth, key: event.target.value })} /><input aria-label={named('API key value')} type='password' autoComplete='off' className={fieldClass} placeholder='Value' value={auth.value} onChange={(event) => onChange({ ...auth, value: event.target.value })} /><select aria-label={named('API key location')} className={fieldClass} value={auth.in} onChange={(event) => onChange({ ...auth, in: event.target.value as 'header' | 'query' | 'cookie' })}><option value='header'>Header</option><option value='query'>Query</option><option value='cookie' disabled={!supports('cookies')}>Cookie (API host)</option></select></div>}
    {auth.type === 'hawk' && <div className='grid gap-2'><input aria-label={named('Hawk id')} className={fieldClass} placeholder='ID' value={auth.id} onChange={(event) => onChange({ ...auth, id: event.target.value })} /><input aria-label={named('Hawk key')} type='password' autoComplete='off' className={fieldClass} placeholder='Key' value={auth.key} onChange={(event) => onChange({ ...auth, key: event.target.value })} /><select aria-label={named('Hawk algorithm')} className={fieldClass} value={auth.algorithm || 'sha256'} onChange={(event) => onChange({ ...auth, algorithm: event.target.value as 'sha1' | 'sha256' })}><option value='sha256'>SHA-256</option><option value='sha1'>SHA-1</option></select><input aria-label={named('Hawk ext')} className={fieldClass} placeholder='ext (optional)' value={auth.ext || ''} onChange={(event) => onChange({ ...auth, ext: event.target.value })} /></div>}
    {auth.type === 'ntlm' && <div className='grid gap-2'><input aria-label={named('NTLM username')} className={fieldClass} placeholder='Username' value={auth.username} onChange={(event) => onChange({ ...auth, username: event.target.value })} /><input aria-label={named('NTLM password')} type='password' autoComplete='off' className={fieldClass} placeholder='Password' value={auth.password} onChange={(event) => onChange({ ...auth, password: event.target.value })} /><input aria-label={named('NTLM domain')} className={fieldClass} placeholder='Domain (optional)' value={auth.domain || ''} onChange={(event) => onChange({ ...auth, domain: event.target.value })} /><input aria-label={named('NTLM workstation')} className={fieldClass} placeholder='Workstation (optional)' value={auth.workstation || ''} onChange={(event) => onChange({ ...auth, workstation: event.target.value })} /></div>}
    {auth.type === 'oauth1' && <div className='grid gap-2'><input aria-label={named('OAuth1 consumer key')} className={fieldClass} placeholder='Consumer key' value={auth.consumerKey} onChange={(event) => onChange({ ...auth, consumerKey: event.target.value })} /><input aria-label={named('OAuth1 consumer secret')} type='password' autoComplete='off' className={fieldClass} placeholder='Consumer secret' value={auth.consumerSecret} onChange={(event) => onChange({ ...auth, consumerSecret: event.target.value })} /><input aria-label={named('OAuth1 token')} className={fieldClass} placeholder='Token (optional)' value={auth.token || ''} onChange={(event) => onChange({ ...auth, token: event.target.value })} /><input aria-label={named('OAuth1 token secret')} type='password' autoComplete='off' className={fieldClass} placeholder='Token secret (optional)' value={auth.tokenSecret || ''} onChange={(event) => onChange({ ...auth, tokenSecret: event.target.value })} /><select aria-label={named('OAuth1 signature method')} className={fieldClass} value={auth.signatureMethod || 'HMAC-SHA1'} onChange={(event) => onChange({ ...auth, signatureMethod: event.target.value as 'HMAC-SHA1' | 'HMAC-SHA256' | 'PLAINTEXT' })}><option>HMAC-SHA1</option><option>HMAC-SHA256</option><option>PLAINTEXT</option></select><input aria-label={named('OAuth1 realm')} className={fieldClass} placeholder='Realm (optional)' value={auth.realm || ''} onChange={(event) => onChange({ ...auth, realm: event.target.value })} /></div>}
    {auth.type === 'awsv4' && <div className='grid gap-2'><input aria-label={named('AWS access key')} className={fieldClass} placeholder='Access key' value={auth.accessKey} onChange={(event) => onChange({ ...auth, accessKey: event.target.value })} /><input aria-label={named('AWS secret key')} type='password' autoComplete='off' className={fieldClass} placeholder='Secret key' value={auth.secretKey} onChange={(event) => onChange({ ...auth, secretKey: event.target.value })} /><input aria-label={named('AWS session token')} type='password' autoComplete='off' className={fieldClass} placeholder='Session token (optional)' value={auth.sessionToken || ''} onChange={(event) => onChange({ ...auth, sessionToken: event.target.value })} /><input aria-label={named('AWS region')} className={fieldClass} placeholder='Region' value={auth.region} onChange={(event) => onChange({ ...auth, region: event.target.value })} /><input aria-label={named('AWS service')} className={fieldClass} placeholder='Service' value={auth.service} onChange={(event) => onChange({ ...auth, service: event.target.value })} /></div>}
    {label && hostType && <p role={supports(hostType) ? 'status' : 'alert'} className='text-[11px] opacity-75'>{supports(hostType) ? 'This authorization mode is executed by the FlexDoc API host.' : hostExecution?.available ? 'Requests run from your API server, but this authorization mode is not supported.' : 'API-host execution is unavailable on this documentation server.'}</p>}
  </div>;
};