    ...
    <div className='flex flex-col gap-4'>
      ...
      {effectiveRequestTab === 'authorization' && <div role='tabpanel' id={requestPanelId('authorization')} aria-labelledby={requestTabId('authorization')} className='space-y-3'>
        <label className='text-sm font-medium'>Authorization
          <select aria-label='Authorization type' className={smallFieldClass} value={draft.auth?.type || 'none'} onChange={(e) => setAuthType(e.target.value as HttpAuth['type'])}>
        ...
        {draft.auth?.type === 'bearer' && <input aria-label='Bearer token' type='password' autoComplete='off' className={fullFieldClass} value={draft.auth.token} onChange={(e) => { const token = e.target.value; setDraft((current) => ({ ...current, auth: { type: 'bearer', token } })); }} />}
        {(density === 'advanced' || draft.auth?.type === 'oauth2') && draft.auth?.type === 'oauth2' && <OAuthEditor auth={draft.auth} fieldClass={`${fullFieldClass} text-sm`} label='' onChange={(auth) => setDraft((current) => ({ ...current, auth }))} />}
        {draft.auth?.type === 'basic' && <div className='flex gap-2'><input aria-label='Basic auth username' className={fullFieldClass} placeholder='Username' value={draft.auth.username} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'basic' }>), username: e.target.value } }))} /><input aria-label='Basic auth password' type='password' autoComplete='off' className={fullFieldClass} placeholder='Password' value={draft.auth.password} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'basic' }>), password: e.target.value } }))} /></div>}
        {draft.auth?.type === 'apiKey' && <div className='flex gap-2'><input aria-label='API key name' className={fullFieldClass} placeholder='Key name' value={draft.auth.key} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'apiKey' }>), key: e.target.value } }))} /><input aria-label='API key value' type='password' autoComplete='off' className={fullFieldClass} placeholder='Value' value={draft.auth.value} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'apiKey' }>), value: e.target.value } }))} /><select aria-label='API key location' className={smallFieldClass} value={draft.auth.in} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'apiKey' }>), in: e.target.value as 'header' | 'query' | 'cookie' } }))}><option value='header'>Header</option><option value='query'>Query</option><option value='cookie' disabled={!supportsHostCapability('cookies')}>Cookie (API host)</option></select></div>}
        {draft.auth?.type === 'digest' && <div className='grid grid-cols-2 gap-2'><input aria-label='Digest username' className={fieldClass} placeholder='Username' value={draft.auth.username} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'digest' }>), username: e.target.value } }))} /><input aria-label='Digest password' type='password' autoComplete='off' className={fieldClass} placeholder='Password' value={draft.auth.password} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'digest' }>), password: e.target.value } }))} /></div>}
        {draft.auth?.type === 'hawk' && <div className='grid gap-2 sm:grid-cols-2'><input aria-label='Hawk id' className={fieldClass} placeholder='ID' value={draft.auth.id} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'hawk' }>), id: e.target.value } }))} /><input aria-label='Hawk key' type='password' autoComplete='off' className={fieldClass} placeholder='Key' value={draft.auth.key} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'hawk' }>), key: e.target.value } }))} /><select aria-label='Hawk algorithm' className={fieldClass} value={draft.auth.algorithm || 'sha256'} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'hawk' }>), algorithm: e.target.value as 'sha1' | 'sha256' } }))}><option value='sha256'>SHA-256</option><option value='sha1'>SHA-1</option></select><input aria-label='Hawk ext' className={fieldClass} placeholder='ext (optional)' value={draft.auth.ext || ''} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'hawk' }>), ext: e.target.value } }))} /></div>}
        {draft.auth?.type === 'ntlm' && <div className='grid gap-2 sm:grid-cols-2'><input aria-label='NTLM username' className={fieldClass} placeholder='Username' value={draft.auth.username} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'ntlm' }>), username: e.target.value } }))} /><input aria-label='NTLM password' type='password' autoComplete='off' className={fieldClass} placeholder='Password' value={draft.auth.password} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'ntlm' }>), password: e.target.value } }))} /><input aria-label='NTLM domain' className={fieldClass} placeholder='Domain (optional)' value={draft.auth.domain || ''} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'ntlm' }>), domain: e.target.value } }))} /><input aria-label='NTLM workstation' className={fieldClass} placeholder='Workstation (optional)' value={draft.auth.workstation || ''} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'ntlm' }>), workstation: e.target.value } }))} /></div>}
        {draft.auth?.type === 'oauth1' && <div className='grid gap-2 sm:grid-cols-2'><input aria-label='OAuth1 consumer key' className={fieldClass} placeholder='Consumer key' value={draft.auth.consumerKey} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'oauth1' }>), consumerKey: e.target.value } }))} /><input aria-label='OAuth1 consumer secret' type='password' autoComplete='off' className={fieldClass} placeholder='Consumer secret' value={draft.auth.consumerSecret} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'oauth1' }>), consumerSecret: e.target.value } }))} /><input aria-label='OAuth1 token' className={fieldClass} placeholder='Token (optional)' value={draft.auth.token || ''} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'oauth1' }>), token: e.target.value } }))} /><input aria-label='OAuth1 token secret' type='password' autoComplete='off' className={fieldClass} placeholder='Token secret (optional)' value={draft.auth.tokenSecret || ''} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'oauth1' }>), tokenSecret: e.target.value } }))} /><select aria-label='OAuth1 signature method' className={fieldClass} value={draft.auth.signatureMethod || 'HMAC-SHA1'} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'oauth1' }>), signatureMethod: e.target.value as 'HMAC-SHA1' | 'HMAC-SHA256' | 'PLAINTEXT' } }))}><option>HMAC-SHA1</option><option>HMAC-SHA256</option><option>PLAINTEXT</option></select><input aria-label='OAuth1 realm' className={fieldClass} placeholder='Realm (optional)' value={draft.auth.realm || ''} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'oauth1' }>), realm: e.target.value } }))} /></div>}
        {draft.auth?.type === 'awsv4' && <div className='grid gap-2 sm:grid-cols-2'><input aria-label='AWS access key' className={fieldClass} placeholder='Access key' value={draft.auth.accessKey} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'awsv4' }>), accessKey: e.target.value } }))} /><input aria-label='AWS secret key' type='password' autoComplete='off' className={fieldClass} placeholder='Secret key' value={draft.auth.secretKey} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'awsv4' }>), secretKey: e.target.value } }))} /><input aria-label='AWS session token' type='password' autoComplete='off' className={fieldClass} placeholder='Session token (optional)' value={draft.auth.sessionToken || ''} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'awsv4' }>), sessionToken: e.target.value } }))} /><input aria-label='AWS region' className={fieldClass} placeholder='Region' value={draft.auth.region} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'awsv4' }>), region: e.target.value } }))} /><input aria-label='AWS service' className={fieldClass} placeholder='Service' value={draft.auth.service} onChange={(e) => setDraft((current) => ({ ...current, auth: { ...(current.auth as Extract<HttpAuth, { type: 'awsv4' }>), service: e.target.value } }))} /></div>}
