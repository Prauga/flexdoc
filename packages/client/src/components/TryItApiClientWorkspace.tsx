...
interface ApiClientSession extends OpenApiApiClientSession {
  id: number;
}
...
export const TryItApiClientWorkspace: React.FC<Props> = ({ spec, path, method, theme, options, onRequestChange }) => {
  ...
  const openInApiClient = (nextSession: OpenApiApiClientSession) => {
    nextSessionId.current += 1;
    setSession({ id: nextSessionId.current, ...nextSession });
  };
  ...
  return <>
    ...
    {session && <section className='mt-6' aria-labelledby='api-client-heading'>
