...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  theme = 'light',
  expandedTags: controlledExpandedTags,
  onExpandedTagsChange,
  messages,
}) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [localExpandedTags, setLocalExpandedTags] = useState<Set<string>>(new Set(controlledExpandedTags?.length ? controlledExpandedTags : ['default']));
  const searchId = useId();
  const rootRef = useRef<HTMLDivElement>(null);
  const expandedTags = controlledExpandedTags ? new Set(controlledExpandedTags) : localExpandedTags;

  useEffect(() => {
    if (!selectedEndpoint) return;
    requestAnimationFrame(() => {
      rootRef.current?.querySelector<HTMLElement>('[data-selected-endpoint="true"]')?.scrollIntoView({ block: 'nearest' });
    });
  }, [selectedEndpoint]);
  ...
  const toggleTag = (tag: string) => {
    ...
    if (!controlledExpandedTags) setLocalExpandedTags(next);
    onExpandedTagsChange?.([...next]);
  };

  return (
    <div ref={rootRef} className={`flex h-full w-full min-w-0 flex-col ${sidebarClasses}`} style={{ backgroundColor: 'var(--flexdoc-sidebar-bg)', color: 'var(--flexdoc-sidebar-text)' }}>
      <div className={`border-b p-4 ${theme === 'dark' ? 'border-gray-800' : 'border-gray-100'}`}>
        <label className='sr-only' htmlFor={searchId}>{messages?.searchEndpoints || 'Search endpoints'}</label>
        <div className='relative'>
          <Search className='absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400' />
          <input id={searchId} type='search' placeholder={messages?.searchEndpointsPlaceholder || 'Search endpoints...'} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={`w-full rounded-lg border py-2 pl-10 pr-4 text-sm transition-colors ${searchInputClasses}`} />
        </div>
      </div>
      ...
      <div className='min-h-0 flex-1 overflow-y-auto overscroll-contain'>
        <div className='p-4'>
          <div className='mb-6'>
            <h3 className={`mb-2 text-xs font-semibold uppercase tracking-wider ${sectionHeaderClasses}`}>{messages?.apiInformation || 'API Information'}</h3>
            <div className='space-y-3'>
              <div className={`rounded-lg border p-3 ${cardClasses}`}>
                ...
                <p className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{messages?.versionLabel || 'Version'} {spec.info.version}</p>
              </div>
              {spec.servers && spec.servers.length > 0 && (
                <div className={`rounded-lg border p-3 ${cardClasses}`}>
                  <div className='mb-2 flex items-center gap-2'><Server className='h-4 w-4 text-gray-400' /><span className='text-sm font-medium'>{messages?.servers || 'Servers'}</span></div>
          ...
          <div>
            <h3 className={`mb-3 text-xs font-semibold uppercase tracking-wider ${sectionHeaderClasses}`}>{messages?.endpoints || 'Endpoints'}</h3>
            {Object.keys(filteredGroups).length === 0 && <p className={`px-2 py-4 text-sm ${sectionHeaderClasses}`}>{messages?.noEndpointsMatch || 'No endpoints match your search.'}</p>}
            {Object.entries(filteredGroups).map(([tag, endpoints]) => {
              const expanded = searching || expandedTags.has(tag);
              return <div key={tag} className='mb-4'>
                <button type='button' aria-expanded={expanded} onClick={() => toggleTag(tag)} className={`flex min-h-11 w-full items-center gap-2 rounded-lg p-2 text-left transition-colors ${theme === 'dark' ? 'hover:bg-gray-800/50 text-white' : 'hover:bg-gray-50 text-gray-700'}`}>
                  ...
                  <span className='min-w-0 truncate text-sm font-medium'>{tag === 'default' ? (messages?.generalTag || 'General') : tag}</span>
                ...
                {expanded && (
                  <div className='ml-4 mt-1.5 space-y-1.5 sm:ml-6'>
                    {endpoints.map(({ path, method, operation }) => {
                      ...
                      return <button type='button' key={`${method}-${path}`} data-selected-endpoint={isSelected ? 'true' : undefined} onClick={() => onEndpointSelect(path, method)} className={`mb-2 min-h-11 w-full rounded-lg border p-2 text-left transition-all ${isSelected ? theme === 'dark' ? 'bg-blue-900/20 border-blue-800/50 shadow-sm' : 'bg-blue-50 border-blue-100 shadow-sm' : theme === 'dark' ? 'hover:bg-gray-800/50 border-transparent hover:border-gray-700' : 'hover:bg-gray-50 border-transparent hover:border-gray-100'}`} style={isSelected ? { color: 'var(--flexdoc-sidebar-active-text)' } : undefined}>
