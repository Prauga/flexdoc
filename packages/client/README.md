...
Canonical React components and utilities for rendering self-hosted OpenAPI documentation and API Client workflows.
...
Workspace authorization can be defined at the collection, folder, or request level. Newly created workspace request drafts default to **Inherit from parent**; existing saved drafts with explicit **No auth** remain unchanged. Resolution walks the selected leaf folder, its ancestors, and then the collection. The nearest explicit auth setting wins, and an explicit **No auth** setting stops inheritance. Collection, folder, and request auth supports bearer tokens, OAuth 2.0, Basic auth, API keys in headers/query/cookies, Digest, Hawk, OAuth 1.0, AWS Signature V4, and NTLM intent. Browser-safe modes execute normally; cookie API keys and advanced schemes are enabled only when the serving adapter advertises the matching API-host capability. OAuth supports manual access-token entry, Authorization Code with PKCE, Client Credentials, legacy Password and Implicit grants, and refresh-token reuse. Token acquisition and refresh are explicit actions: **Send** never opens an OAuth popup, acquires a missing token, or refreshes an expired token automatically, and `expires_in` is currently informational rather than persisted for auto-refresh. Token endpoint client authentication can send `client_id`/`client_secret` in the form body (the default for compatibility) or use an HTTP Basic `Authorization` header. Interactive popup grants require their callback to return to the FlexDoc page origin, and token endpoints must allow browser CORS. Client secrets entered in the browser are not confidential and should only be used with providers that permit browser-based clients. History retains collection and folder identity so inherited-auth requests replay in their original context when that context still exists.

When FlexDoc is served by an adapter with API-host execution enabled, `ApiClient`/`ApiClientWorkspace` receive a capability description rather than backend secrets. A request that needs Digest, Hawk, OAuth 1.0, AWS Signature V4, a cookie API key, a configured client certificate, or the host cookie jar is routed through that adapter-owned executor; browser-only installations keep those controls unavailable. The Node backend implementation is explicit opt-in and is documented in [`docs/host-execution.md`](../../docs/host-execution.md). NTLM is preserved in the canonical model but is not advertised by the Node executor in 2.9.5.

Explicit authentication fields follow the workspace **Credential storage** setting, which covers request, folder, and collection auth. A newly created workspace defaults to **Session only**: credential material is held in a per-tab `sessionStorage` vault, stripped from the IndexedDB snapshot, and restored when that tab reopens the workspace. **Remember on this browser** persists it in IndexedDB as entered. **Never store** keeps it in memory only, clears any existing tab vault, and strips persisted values. A workspace saved before this setting existed records no scope and keeps the remember behaviour it already had. Stripping removes only the secret itself; non-secret routing and signing configuration such as usernames, key names, regions, and consumer keys is retained so a saved request reopens intact.

The scope does not extend beyond those auth fields. Saved request drafts, scripts, collection variables, and environment values are persisted as entered, so a bearer token typed into a variable, a raw `Authorization` header, or a secret referenced from a script is stored regardless of the selected scope. Request **history** is stricter at the IndexedDB boundary: common credential/session request and response headers such as `Authorization`, `Cookie`, `Set-Cookie`, API-key headers, and access/security-token headers are replaced with `[REDACTED]` before the workspace snapshot is written. Request and response bodies remain persisted by default. Turn off **Store API-host bodies in history** in the Recent history panel to omit request/response bodies from persisted API-host entries; legacy entries whose original transport cannot be proven are treated conservatively the same way, while positively identified Browser executions keep their bodies. The privacy setting changes only the persisted snapshot—the current in-memory session remains complete. IndexedDB is scoped by browser origin, but FlexDoc does not encrypt saved requests, auth fields, variables, scripts, or other workspace state and does not create an additional security boundary between persistence keys on the same origin. Only persist secrets on origins and devices you trust.
