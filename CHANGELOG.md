...
- Opening that finding shows the registered route and can send it from the API Client. The URL is the runtime path on the registering origin, not on a server base path. The request asks to prefer the API host only when host execution is available and the server has not opted out. It is not treated as an OpenAPI operation.
